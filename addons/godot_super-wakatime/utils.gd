func plugin_print(msg: String) -> void:
    """Print message from plugin"""
    print("[Godot_Super-Wakatime]: %s" % msg)
    
func plugin_print_err(err: String) -> void:
...
func wakatime_cli_exists(wakatime_cli: String) -> bool:
    """Return if wakatime cli tool exists already"""
    return FileAccess.file_exists(wakatime_cli)
    
func wakatime_zip_exsits(wakatime_zip: String) -> bool:
